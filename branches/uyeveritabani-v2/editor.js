...
function editor_updateToolbar(objname,action) {
  ...
  if (action == "enable" || action == "disable") {
    var tbItems = new Array('FontName','FontSize','FontStyle');                           // ekleme pulldowns
    for (var btnName in config.btnList) { tbItems.push(config.btnList[btnName][0]); } // ekleme buttons
...
function editor_filterOutput(objname) {
  ...
  var filterTag = function(tagBody,tagName,tagAttr) {
    ...
    // ekleme additional tag filtering here
