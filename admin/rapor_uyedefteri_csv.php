<?php
	...
	if( isset($_GET["x"]) && isset($_GET["y"]) ) { // Aralik verilmisse csv ciktisini bastir
		...
		header("Content-Disposition: attachment; filename=uyedefteri-". date("dmY"). ".csv;");
		...
		$y = intval($_GET["y"]);
       		...
       		$Sorgu = "SELECT odemeler.uye_id,uyeler.uye_ad,uyeler.uye_soyad,odemeler.miktar,odemeler.tarih FROM odemeler,uyeler WHERE odemeler.uye_id BETWEEN ". $x ." AND ". $y ." AND odemeler.tur = 'aidat' AND uyeler.uye_id=odemeler.uye_id ORDER BY odemeler.uye_id,odemeler.tarih";
